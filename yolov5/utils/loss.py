...
class ComputeLoss:
    ...
    def build_targets(self, p, targets):
        ...
        tangle = []
        gain = torch.ones(8, device=targets.device)  # normalized to gridspace gain        ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)
